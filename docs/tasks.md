...
*Já concluído (`ng new` com Tailwind, sem SSR/SSG, Copilot e variáveis de ambiente configuradas).*
...
- [ ] `src/app/features/auth/index.schema.ts` — schemas Zod `loginRequestSchema`, `loginResponseSchema` e tipos inferidos
- [ ] `src/app/features/auth/index.service.ts` — `AuthService` com chamada `POST /auth/login` e integração com `SessionService`
- [ ] `src/app/features/auth/index.service.spec.ts` — testes unitários do serviço de autenticação
- [ ] `src/app/features/auth/login/index.component.ts` — formulário de login (Reactive Forms, mensagem genérica de credencial inválida)
- [ ] `src/app/features/auth/login/index.component.html` — template da tela de login
- [ ] `src/app/features/auth/login/index.component.spec.ts` — testes unitários da tela de login

---

## Fase 4 — Feature: Traceability (pública)

*Depende de: Fase 2 (shared components). Não depende de Fase 1/3 (rota pública, paralelizável com Auth).*
...
*Depende de: Fases 1 e 2. Cada feature é independente das outras — ideal para divisão da dupla.*
...
*Depende de: todas as features da Fase 6 existirem para o roteamento fazer sentido de ponta a ponta.*
